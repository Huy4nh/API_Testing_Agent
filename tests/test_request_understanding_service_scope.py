...
class FakeScopeResolutionAgent:
    ...
    def decide(self, *, raw_text: str, forced_target_name: str, operation_hints: list[dict]):
...
def test_general_request_without_specific_function_becomes_all_scope():
    service = RequestUnderstandingService(
        nl_interpreter=DummyInterpreter(),
    ...
    assert result.canonical_command == "test target hello_world"
...
def test_specific_function_request_maps_to_exact_operation():
    service = RequestUnderstandingService(
        ...
        scope_resolution_agent=FakeScopeResolutionAgent(
            ScopeResolutionDecision(
                scope_mode="specific",
                matched_operation_id="image_generate_img_post",
                matched_path="/img",
                matched_method="POST",
    ...
    assert result.canonical_command == "test target hello_world /img POST"
    assert result.plan.target_name == "hello_world"
